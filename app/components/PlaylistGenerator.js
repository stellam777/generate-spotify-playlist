...
const PlaylistGenerator = (props) => {
  ...
  const orderDropdownResults = () => {
    let copiedSearchResults = [...searchResults].map(({ name, id }) => ({ name, id }));
    const regex = new RegExp(`^${searchString}`, 'i');
    copiedSearchResults = copiedSearchResults.sort().filter(artist => regex.test(artist.name));
    ...
    <div className="container">
      ...
      <div>
          <div className="form-group">
            <label>Enter an artist below to generate a new recommended playlist</label>
            <input
              type="text"
              name="search"
              ...
              value={searchString}
            />
             {isSelected && <SelectedSearch recResults={recResults} selectedSearchStrings={selectedSearchStrings} setSelectedSearchStrings={setSelectedSearchStrings}/>}
            <ul>
              {searchString &&
                autoCompleteText.map(artist => <li key={artist.id} onClick={() => selectArtist(artist)}>{artist.name}</li>)
              }
            </ul>
            {selectedSearchStrings.length ? <Playlist recResults={recResults} /> : ''}
          </div>
      </div>
    </div>
  )
